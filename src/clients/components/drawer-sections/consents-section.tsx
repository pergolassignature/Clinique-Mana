...
export function ConsentsSection({ client }: ConsentsSectionProps) {
  ...
  const getStatusBadge = (status: ConsentStatus) => {
    switch (status) {
      ...
      case 'expired':
        return <Badge className="bg-honey-100 text-honey-700 text-xs">Expiré</Badge>
      case 'missing':
            ...
            <div className="p-3 rounded-lg bg-wine-50 border border-wine-200">
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm text-wine-800">
                  Un ou plusieurs consentements nécessitent votre attention.
                </p>
              </div>
            </div>
          )}

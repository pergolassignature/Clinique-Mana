-- Migration: facturation_fixes
-- Module: facturation
-- Gate: Fixes file opening fee default, improves error handling
-- Created: 2026-01-23

-- =============================================================================
-- FIX: Update file opening fee default from $25 to $35
-- =============================================================================
...
begin
  ...
  if v_service_id is not null then
    ...
    order by sp.created_at desc -- Use most recent price if multiple
    limit 1;
  ...
  if p_add_file_opening_fee then
    ...
    if v_fof_service_id is not null then
      ...
      -- Default to $35.00 (3500 cents) if not set in service_prices
      v_fof_price_cents := coalesce(v_fof_price_cents, 3500);
  ...
  -- Log audit entry (using SECURITY DEFINER context)
  insert into public.invoice_audit_log (
    ...
    'created',
    jsonb_build_object(
      'totalCents', v_invoice_total_cents,
      'lineItemCount', v_line_order,
      'hasFileOpeningFee', p_add_file_opening_fee
    )
  );
...
comment on function public.create_invoice_with_line_item(uuid, uuid, text, text, boolean) is
  'Creates an invoice with initial service line item and optional file opening fee ($35 default). Includes validation and audit logging.';

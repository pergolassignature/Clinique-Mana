-- Migration: facturation_hourly_prorata
-- Module: facturation
-- Purpose: Update create_invoice_with_line_item to handle by_profession_hourly_prorata pricing
-- Created: 2026-01-25
--
-- Services with pricing_model = 'by_profession_hourly_prorata' use profession_category_rates
-- for minute-based billing instead of service_prices.
...
declare
  ...
  v_appointment_profession_category_key text;
  v_hourly_rate_cents int;
  v_tax_tps_cents int;
  v_tax_tvq_cents int;
  v_total_cents int;
  v_line_order int := 0;
...
begin
  ...
  -- Get appointment details including cancellation info and pricing model
  select
    a.professional_id,
    a.service_id,
    a.duration_minutes,
    a.status,
    a.profession_category_key,
  ...
  into
    ...
    v_appointment_profession_category_key,
    v_cancellation_fee_applied,
    v_cancellation_fee_percent,
    v_service_name,
    v_service_key,
    v_service_pricing_model,
    v_is_taxable
  ...
  -- If appointment doesn't have profession_category_key, get from professional's primary profession
  if v_appointment_profession_category_key is null then
    select pt.profession_category_key
    into v_appointment_profession_category_key
    from public.professional_professions pp
    join public.profession_titles pt on pt.key = pp.profession_title_key
    where pp.professional_id = v_professional_id
  ...
  if v_service_id is not null then
    ...
    v_profession_category_key := v_appointment_profession_category_key;

    -- Get price based on pricing model
    if v_service_pricing_model = 'by_profession_hourly_prorata' then
      -- Hourly prorata: use profession_category_rates
      -- Price = (hourly_rate / 60) * duration_minutes
      select pcr.hourly_rate_cents
      into v_hourly_rate_cents
      from public.profession_category_rates pcr
      where pcr.profession_category_key = v_appointment_profession_category_key
        and pcr.is_active = true
      limit 1;

      if v_hourly_rate_cents is not null then
        -- Per-minute rate
        v_unit_price_cents := round(v_hourly_rate_cents::numeric / 60);
        -- Total price = per-minute * duration
        v_price_cents := v_unit_price_cents * v_duration_minutes;
        -- For hourly prorata, quantity = duration in minutes
        v_quantity_unit := 'minute';
        v_quantity := v_duration_minutes;
      end if;

      -- Check category tax status for hourly prorata services
      if not v_is_taxable then
        select pc.tax_included
        into v_is_taxable
        from public.profession_categories pc
        where pc.key = v_appointment_profession_category_key;

        v_is_taxable := coalesce(v_is_taxable, false);
      end if;
    else
      -- Standard pricing: use service_prices
      ...
      where sp.service_id = v_service_id
        ...
        and (sp.profession_category_key = v_appointment_profession_category_key
             or sp.profession_category_key is null)
      order by
        case when sp.profession_category_key = v_appointment_profession_category_key then 0 else 1 end,
        sp.created_at desc
      limit 1;

      v_price_cents := coalesce(v_price_cents, 0);
      v_unit_price_cents := v_price_cents;
      v_quantity := 1;
      v_quantity_unit := 'unit';
    end if;
    ...
    if v_appointment_status = 'cancelled' then
      ...
      if v_cancellation_fee_applied and v_cancellation_fee_percent > 0 then
        -- Calculate cancellation fee based on percentage of total price
  ...
  insert into public.invoice_audit_log (
    ...
    jsonb_build_object(
      ...
      'pricing_model', v_service_pricing_model
    )
  );
...
comment on function public.create_invoice_with_line_item(uuid, uuid, text, text, boolean) is
  'Creates an invoice with line items. Handles standard pricing (service_prices) and hourly prorata pricing (profession_category_rates). For cancelled appointments with fee, bills cancellation fee only. Sets created_by to current user.';
